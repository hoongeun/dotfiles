function rvm --description='Ruby enVironment Manager'
  ...
  bash -c 'PATH=$GEM_HOME/bin:$PATH;RVM=$(whereis rvm | sed "s/rvm://");source $(if test $RVMB; then echo $RVM | sed "s/rvm/rvm\/scripts\/rvm/"; else echo ~/.rvm/scripts/rvm; fi); rvm "$@"; status=$?; env > "$0"; exit $status' $env_file $argv
